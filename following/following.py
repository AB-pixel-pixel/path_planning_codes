...
class DynamicFollowingDemo:
    def __init__(self, width=100, height=100, fov=90):
        ...
        # 人物移动历史（用于预测）
        self.target_history = deque(maxlen=15)  # 增加历史记录长度
        ...
        self.continuous_visible_steps = 0  # 连续可见步数
    ...
    def _should_replan(self):
        ...
        # 目标可见且连续可见时间较短，频繁重新规划
        if self.target_visible and self.continuous_visible_steps < 20:
            return self.step_count % 2 == 0  # 每2步重新规划
        
        # 目标可见且稳定跟随，减少重新规划频率
        if self.target_visible:
            return self.step_count % 5 == 0  # 每5步重新规划
    ...
    def step(self):
        ...
        if self.planned_path and len(self.planned_path) > 1:
            ...
            if current_dist < FOLLOW_DISTANCE - 5:
                step_size = 1.0  # 太近了，慢速移动
            elif current_dist > FOLLOW_DISTANCE + 10:
                step_size = 2.5  # 太远了，加速追赶
            else:
                step_size = 2.0  # 正常速度
    ...
    def render_frame(self, step_idx=None):
        ...
        ax_left.set_title('Ground Truth Map (Extended Perception)', fontsize=14, fontweight='bold')
        ...
        ax_right.set_title(f'Semantic Map - Smart Following', 
...
def run_demo_with_gif_export():
    ...
    while step < max_steps:
        ...
        if step % 50 == 0:
            ...
            print(f"Step {step} - Target {'VISIBLE' if demo.target_visible else 'LOST'} - Avg Distance: {avg_distance:.1f}m")
    ...
    # 保存GIF
    gif_filename = 'robot_following_enhanced.gif'
    print(f"\n正在保存GIF...")
