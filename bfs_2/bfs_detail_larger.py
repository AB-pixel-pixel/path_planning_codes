...
def create_bfs_animation(visualizer: BFSVisualizer, graph_layout: Dict[str, Tuple[float, float]], 
                        start: str, target: Optional[str] = None, interval=1000, 
                        save_as=None, save_frames_dir=None):
    """
    ...
    save_frames_dir: directory to save individual frames (e.g., 'bfs_frames')
    """
    steps = visualizer.steps
    
    # Create directory for frames if specified
    if save_frames_dir:
        os.makedirs(save_frames_dir, exist_ok=True)
        print(f"Saving frames to directory: {save_frames_dir}/")
    ...
    def update(frame):
        ...
        for node, (x1, y1) in graph_layout.items():
            for neighbor in visualizer.graph.get(node, []):
                ...
                ax_main.plot([x1, x2], [y1, y2], 'k-', linewidth=3, alpha=0.15, zorder=0)
        ...
        for child, parent in step['parent'].items():
            if parent is not None:
                ...
                ax_main.plot([x1, x2], [y1, y2], 'blue', linewidth=4, alpha=0.5, zorder=1)
        ...
        if step['current'] and step['exploring']:
            ...
            for neighbor in step['exploring']:
                x2, y2 = graph_layout[neighbor]
                ax_main.plot([x1, x2], [y1, y2], 'orange', linewidth=5, alpha=0.7, zorder=2)
        
        # Draw nodes
        for node, (x, y) in graph_layout.items():
            if node == step['current']:
                # Current node (orange with pulse)
                circle = patches.Circle((x, y), 0.16, color='orange', ec='black', linewidth=4, zorder=5)
                pulse_circle = patches.Circle((x, y), 0.24, color='orange', ec='orange', 
                                            linewidth=3, alpha=0.3, zorder=4)
                ax_main.add_patch(pulse_circle)
            elif node in step['just_added']:
                # Just added nodes (yellow)
                circle = patches.Circle((x, y), 0.16, color='yellow', ec='black', linewidth=4, zorder=5)
            elif node in step['visited']:
                # Visited node (light green)
                circle = patches.Circle((x, y), 0.16, color='lightgreen', ec='black', linewidth=4, zorder=3)
            else:
                # Unvisited node (light gray)
                circle = patches.Circle((x, y), 0.16, color='lightgray', ec='black', linewidth=4, zorder=2)
            
            ax_main.add_patch(circle)
            ax_main.text(x, y, node, ha='center', va='center', fontsize=28, fontweight='bold', zorder=6)
        ...
        legend_elements = [
            ...
            plt.Line2D([0], [0], color='blue', linewidth=4, alpha=0.5, label='Parent Link')
        ]
        ax_main.legend(handles=legend_elements, loc='upper left', fontsize=15)
        
        # Information panel with larger fonts
        ax_info.axis('off')
        info_text = []
        ...
        # Display info text with larger fonts
        y_pos = 0.95
        for line in info_text:
            if line.startswith('Action:'):
                ax_info.text(0.05, y_pos, line, fontsize=18, fontweight='bold', 
                           transform=ax_info.transAxes, family='monospace')
            elif line.startswith('-'):
                ax_info.text(0.05, y_pos, line, fontsize=14, 
                           transform=ax_info.transAxes, family='monospace', color='gray')
            elif any(line.startswith(x) for x in ['Queue', 'Visited', 'Parent']):
                ax_info.text(0.05, y_pos, line, fontsize=17, fontweight='bold',
                           transform=ax_info.transAxes, family='monospace', color='darkblue')
            else:
                ax_info.text(0.05, y_pos, line, fontsize=15, 
                           transform=ax_info.transAxes, family='monospace')
            y_pos -= 0.045
        
        # Save individual frame if directory specified
        if save_frames_dir:
            frame_filename = os.path.join(save_frames_dir, f'frame_{frame:03d}.png')
            plt.savefig(frame_filename, dpi=150, bbox_inches='tight')
            if frame == 0:
                print(f"Saving frame {frame + 1}/{len(steps)}...", end='')
            elif frame == len(steps) - 1:
                print(f"\rSaved all {len(steps)} frames successfully!")
            elif (frame + 1) % 5 == 0:
                print(f"\rSaving frame {frame + 1}/{len(steps)}...", end='')
...
def create_path_reconstruction_animation(visualizer: BFSVisualizer, 
                                        ...
                                        interval=1200, save_as=None, save_frames_dir=None):
    """
    Create animation showing path reconstruction using parent pointers
    save_frames_dir: directory to save individual frames
    """
    ...
    def update(frame):
        ...
        for node, (x1, y1) in graph_layout.items():
            for neighbor in visualizer.graph.get(node, []):
                x2, y2 = graph_layout[neighbor]
                ax_main.plot([x1, x2], [y1, y2], 'k-', linewidth=3, alpha=0.1, zorder=0)
        ...
        for child, parent in visualizer.parent.items():
            if parent is not None:
                x1, y1 = graph_layout[parent]
                x2, y2 = graph_layout[child]
                ax_main.plot([x1, x2], [y1, y2], 'lightblue', linewidth=3, alpha=0.3, zorder=1)
        ...
        for i in range(len(traced_path) - 1):
            ...
            ax_main.plot([x1, x2], [y1, y2], 'red', linewidth=6, alpha=0.8, zorder=5)
            # Arrow
            dx, dy = x2 - x1, y2 - y1
            ax_main.arrow(x1, y1, dx*0.7, dy*0.7, head_width=0.09, head_length=0.07,
        ...
        for node, (x, y) in graph_layout.items():
            if node == step['current']:
                ...
                circle = patches.Circle((x, y), 0.16, color='red', ec='black', linewidth=4, zorder=6)
                pulse_circle = patches.Circle((x, y), 0.24, color='red', ec='red',
                                            linewidth=3, alpha=0.3, zorder=5)
                ax_main.add_patch(pulse_circle)
            elif node in step['traced']:
                # Already traced
                circle = patches.Circle((x, y), 0.16, color='salmon', ec='black', linewidth=4, zorder=4)
            elif node in visualizer.visited:
                # Visited but not in path
                circle = patches.Circle((x, y), 0.16, color='lightgreen', ec='black', linewidth=3, zorder=2)
            else:
                # Unvisited
                circle = patches.Circle((x, y), 0.16, color='lightgray', ec='black', linewidth=3, zorder=1)
            
            ax_main.add_patch(circle)
            ax_main.text(x, y, node, ha='center', va='center', fontsize=28, fontweight='bold', zorder=7)
        
        ax_main.set_title(f"Path Reconstruction - Step {frame + 1}/{len(recon_steps)}", 
                         fontsize=22, fontweight='bold', pad=25)
        
        # Info panel with larger fonts
        ax_info.axis('off')
        info_text = []
        ...
        # Save individual frame if directory specified
        if save_frames_dir:
            frame_filename = os.path.join(save_frames_dir, f'recon_frame_{frame:03d}.png')
            plt.savefig(frame_filename, dpi=150, bbox_inches='tight')
            if frame == 0:
                print(f"Saving frame {frame + 1}/{len(recon_steps)}...", end='')
            elif frame == len(recon_steps) - 1:
                print(f"\rSaved all {len(recon_steps)} frames successfully!")
            elif (frame + 1) % 3 == 0:
                print(f"\rSaving frame {frame + 1}/{len(recon_steps)}...", end='')
    ...
    if save_as:
        print(f"\nSaving path reconstruction animation to {save_as}...")
        writer = PillowWriter(fps=1)
        anim.save(save_as, writer=writer)
        print(f"Animation saved!")
...
# Create detailed animation with frames saved
anim1 = create_bfs_animation(viz1, layout1, 'A', interval=1500, 
                            save_as='tree_bfs_detailed.gif',
                            save_frames_dir='tree_bfs_frames')
...
print("\nOutput Files:")
print("- GIF animations: *.gif")
print("- Individual frames: *_frames/ directories")
