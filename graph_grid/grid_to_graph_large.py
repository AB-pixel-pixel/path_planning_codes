...
class GridToGraphVisualizer:
    ...
    def _draw_grid_map(self, ax, show_coordinates, highlight_cell, diagonal):
        """Draw grid map"""
        ax.set_title('Grid Map', fontsize=40, fontweight='bold', pad=20)
        ...
        ax.legend(handles=legend_elements, loc='upper left', 
                 bbox_to_anchor=(0, 1), fontsize=25)
    ...
    def _draw_graph_structure(self, ax, diagonal, highlight_cell):
                ...
                if edge not in edges_drawn:
                    ...
                    if is_highlight:
                        ax.plot([x + 0.5, nx + 0.5], [y + 0.5, ny + 0.5],
                               'r-', linewidth=6, alpha=0.8, zorder=5)
                    else:
                        ...
                        if abs(x - nx) + abs(y - ny) == 2:  # Diagonal
                            ...
                            ax.plot([x + 0.5, nx + 0.5], [y + 0.5, ny + 0.5],
                                   'gray', linewidth=4, alpha=0.7, zorder=2)
        ...
        for (x, y) in free_cells:
            if highlight_cell and (x, y) == highlight_cell:
                ...
                ax.plot(x + 0.5, y + 0.5, 'o', color='red', 
                       markersize=40, zorder=10, 
                       markeredgecolor='darkred', markeredgewidth=5)
                ...
                ax.plot(x + 0.5, y + 0.5, 'o', color='#3498DB', 
                       markersize=24, zorder=6,
                       markeredgecolor='#2874A6', markeredgewidth=3)
        ...
        legend_elements = [
            mlines.Line2D([], [], color='#3498DB', marker='o', linestyle='None',
                         markersize=30, markeredgecolor='#2874A6', 
                         markeredgewidth=3, label='Node'),
            mlines.Line2D([], [], color='gray', linewidth=4, 
                         label='4-Connected Edge'),
        ]
        
        if diagonal:
            legend_elements.append(
                mlines.Line2D([], [], color='b', linewidth=3, linestyle='--',
        ...
        if highlight_cell:
            legend_elements.extend([
                mlines.Line2D([], [], color='red', marker='o', linestyle='None',
                             markersize=30, label='Center Node'),
                mlines.Line2D([], [], color='#6BCB77', marker='o', linestyle='None',
                             markersize=30, label='Neighbor Node'),
            ])
        
        ax.legend(handles=legend_elements, loc='upper left', 
                 bbox_to_anchor=(0, 1), fontsize=30)
